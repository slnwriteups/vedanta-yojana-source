...
| Compromised EAS Update credential (`EXPO_TOKEN`) | The publish token is a repository secret used only by `.github/workflows/eas-update.yml`, which holds `permissions: contents: read` and publishes from `main` only; `mobile/app.json`'s `fingerprint` runtime policy limits any published bundle to binaries whose native project actually matches it | An over-the-air update is JavaScript delivered after install: anyone able to publish one reaches installed apps directly, without passing through an APK, a signature check, or a store review. This is the most consequential credential in the project |
| JavaScript that differs from the verified APK | Updates are published only from `main`, only by the workflow above, and only after the mobile test suite passes in that same run; every published update is recorded in the project's EAS update history | Verifying an APK's checksum and certificate no longer accounts for all code the app later runs — see [APK-VERIFICATION.md](APK-VERIFICATION.md) |
